...
const PricingComparison: React.FC = () => {
  ...
  const getPrice = (key: PlanKey) => {
    ...
    if (!pkg) return "–";
    const value = isDanish && pkg.price_dkk != null ? pkg.price_dkk : pkg.price_eur;
  ...
  const rawPlans = planKeys.map((key) => ({
    key,
    name: t(`pricingComparison.plans.${key === 'StarterSite' ? 'start' : key === 'WebApplication' ? 'app' : 'premium'}.name`),
    values: featureKeys.map((fk) => {
      const has = (() => {
        if (key === "StarterSite") return [0, 1].includes(featureKeys.indexOf(fk));
        if (key === "WebApplication") return [0,1,4,5,6,7].includes(featureKeys.indexOf(fk));
        return true;
      })();
      return has ? <FaSquareCheck key={`${key}-${fk}`} size={20} className="text-secondary" /> : "–";
    }),
  }));
